...
import { MutualFund, FixedDeposit, Stock, NpsHolding, EpfAccount } from "../types";
...
export function PortfolioInsights({ mfs, fds, stocks, nps, epfs }: Props) {
  ...
  const totalEPF = epfs.reduce((acc, e) => acc + e.balance, 0);
  const totalPortfolio = totalMFCurrent + totalFD + totalStockCurrent + totalNPSCurrent + totalEPF;
  ...
  const npsPercent = totalPortfolio > 0 ? (totalNPSCurrent / totalPortfolio) * 100 : 0;
  const epfPercent = totalPortfolio > 0 ? (totalEPF / totalPortfolio) * 100 : 0;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div className="h-4 w-full rounded-full flex overflow-hidden mb-6 bg-zinc-800">
          ...
          <div style={{ width: `${npsPercent}%` }} className="bg-cyan-500 h-full transition-all duration-1000" />
          <div style={{ width: `${epfPercent}%` }} className="bg-orange-500 h-full transition-all duration-1000" />
        </div>
        <div className="flex flex-wrap gap-8">
          {([
            { label: "Mutual Funds", dot: "bg-emerald-500", percent: mfPercent, value: totalMFCurrent },
            { label: "Stocks", dot: "bg-purple-500", percent: stockPercent, value: totalStockCurrent },
            { label: "Fixed Deposits", dot: "bg-amber-500", percent: fdPercent, value: totalFD },
            { label: "NPS", dot: "bg-cyan-500", percent: npsPercent, value: totalNPSCurrent },
            { label: "EPF", dot: "bg-orange-500", percent: epfPercent, value: totalEPF },
          ]).map((item) => (
            <div key={item.label}>
              ...
              <div className="text-2xl font-bold text-zinc-100">{formatPercent(item.percent)}</div>
              <div className="text-zinc-500 text-sm mt-1">{formatCurrency(item.value)}</div>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      >
        ...
        <ul className="space-y-4 text-zinc-300 text-sm leading-relaxed">
          <li className="flex items-start gap-3">
            ...
            <p>You currently hold <strong>{mfs.length} mutual fund{mfs.length !== 1 ? 's' : ''}</strong>, <strong>{stocks.length} stock{stocks.length !== 1 ? 's' : ''}</strong>, <strong>{fds.length} fixed deposit{fds.length !== 1 ? 's' : ''}</strong>, <strong>{nps.length} NPS scheme{nps.length !== 1 ? 's' : ''}</strong>, and <strong>{epfs.length} EPF account{epfs.length !== 1 ? 's' : ''}</strong> in your portfolio.</p>
          </li>
          <li className="flex items-start gap-3">
            <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 shrink-0" />
            <p>
              {(mfPercent + stockPercent) > 80 ? "Your portfolio is highly skewed towards market-linked assets. Consider diversifying with fixed-income instruments to reduce volatility." :
               (fdPercent + epfPercent) > 80 ? "Your portfolio is highly conservative. Consider adding equity mutual funds or stocks for better long-term inflation-beating returns." :
