...
import { applyCorporateAction } from "@/app/actions/corporate-actions";
...
import type { CorporateActionType, Contributor, Currency, InstrumentType, PriceSource, TxnType } from "@/lib/types";
  ...
  return (
    <div className="space-y-5">
      ...
      {row.source !== "npsnav" && row.type !== "mutual_fund" && (
        <CorporateActionsPanel instrumentId={row.id} />
      )}
...
/**
 * Split / bonus. Multiplies the units on every transaction dated before the
 * ex-date; the ₹ amounts stay as they were (a split doesn't change money in).
 */
function CorporateActionsPanel({ instrumentId }: { instrumentId: string }) {
  const router = useRouter();
  ...
  async function apply(e: React.FormEvent) {
    e.preventDefault();
    setBusy(true);
    const result = await applyCorporateAction({ instrumentId, type, exDate, a: na, b: nb });
    setBusy(false);
    if (!result.ok) return void toast.error(result.error);
    toast(`Applied — ${result.adjusted} transaction${result.adjusted === 1 ? "" : "s"} adjusted`);
    setA(""); setB("1"); setExDate(""); setOpen(false);
    router.refresh();
  }
  ...
  return (
    <div className="rounded-(--radius-field) border border-hairline p-3">
      <div className="flex items-center justify-between">
        <span className="eyebrow">Split / bonus</span>
        {!open && (
      ...
      {!open && (
        <p className="mt-2 text-[12px] text-muted">
          Multiplies the units on every transaction before the ex-date.
        </p>
      )}
      ...
      {open && (
        <form onSubmit={apply} className="mt-3 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-[13px]">
            ...
            <span className="text-muted">{type === "split" ? "new for every" : "bonus for every"}</span>
          ...
          <p className="text-[11px] text-muted">
            {factor ? <>Units before that date become ×{Math.round(factor * 1000) / 1000}.</> : "e.g. 10 new for every 1 old = 10:1 split."}
          </p>
